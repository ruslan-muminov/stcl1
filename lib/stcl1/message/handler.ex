defmodule Stcl1.Message.Handler do
  ...
  def handle_message(%{chat_id: chat_id} = user, text) do
    if chat_id in Settings.operators() do
      UpdatesOperator.handle_message(text, chat_id)
    else
      handle_user_message(user, text)
    end
  end

  def handle_user_message(%{state: "banned"}, _text) do
    :dopizdelsya
  end

  def handle_user_message(user, "/start") do
    Sender.send_text_with_menu(user, :first)
  end

  def handle_user_message(user, @q_back) do
    Sender.send_menu(user)
    Users.update_state(user, "idle")
  end

  def handle_user_message(user, @q_address) do
    Sender.send_text_with_menu(user, :q_address)
    Users.update_state(user, "idle")
  end

  def handle_user_message(user, @q_shows_list) do
  ...
  #
  def handle_user_message(user, @q_tickets) do
    Sender.send_tickets_menu(user)
    Users.update_state(user, "tickets")
  end

  def handle_user_message(%{state: "tickets"} = user, @q_buy_ticket_on_event) do
    Sender.send_text_with_menu(user, :q_buy_ticket_on_event)
    Users.update_state(user, "idle")
  end

  def handle_user_message(%{state: "tickets"} = user, @q_unrecieved_ticket) do
    Sender.send_text_with_menu(user, :q_unrecieved_ticket)
    Users.update_state(user, "idle")
  end

  def handle_user_message(%{state: "tickets"} = user, @q_return_ticket) do
  ...
  #
  def handle_user_message(user, @q_show_advice) do
  ...
  def handle_user_message(%{state: "advice"} = user, @q_show_advice_standup) do
  ...
  def handle_user_message(%{state: "advice"} = user, @q_show_advice_experiment) do
  ...
  def handle_user_message(%{state: "advice"} = user, @q_show_advice_panel) do
  ...
  def handle_user_message(%{state: "advice"} = user, @q_show_new_jokes) do
  ...
  def handle_user_message(%{state: "advice"} = user, @q_show_advice_something_else) do
    variants = [[@q_show_advice_yes], [@q_show_advice_no], [@q_back]]
    Sender.send_variants(user, variants, :q_show_advice2)
    Users.update_state(user, "advice")
  end

  def handle_user_message(%{state: "advice"} = user, @q_show_advice_yes) do
  ...
  def handle_user_message(%{state: "advice"} = user, @q_show_advice_no) do
  ...
  def handle_user_message(user, @q_show_date) do
  ...
  # Составы
  #
  def handle_user_message(user, @q_lineup) do
  ...
  def handle_user_message(%{state: "lineup"} = user, @q_lineup_big) do
    message = Storage.read_lineup_text(:big)
    Sender.send_text_with_menu(user, message)
    Users.update_state(user, "idle")
  end

  def handle_user_message(%{state: "lineup"} = user, @q_lineup_tough) do
    message = Storage.read_lineup_text(:tough)
    Sender.send_text_with_menu(user, message)
    Users.update_state(user, "idle")
  end

  def handle_user_message(%{state: "lineup"} = user, @q_lineup_women) do
  ...
  def handle_user_message(user, @q_show_duration) do
    Sender.send_text_with_menu(user, :q_show_duration)
    Users.update_state(user, "idle")
  end

  def handle_user_message(user, @q_order_food_drink) do
    Sender.send_text_with_menu(user, :q_order_food_drink)
    Users.update_state(user, "idle")
  end

  def handle_user_message(user, @q_show_passport) do
    Sender.send_text_with_menu(user, :q_show_passport)
    Users.update_state(user, "idle")
  end

  def handle_user_message(user, @q_child_with_batya) do
    Sender.send_text_with_menu(user, :q_child_with_batya)
    Users.update_state(user, "idle")
  end

  def handle_user_message(user, @q_parking) do
    Sender.send_text_with_menu(user, :q_parking)
    Users.update_state(user, "idle")
  end

  def handle_user_message(user, @q_loyalty_card) do
    Sender.send_text_with_menu(user, :q_loyalty_card)
    Users.update_state(user, "idle")
  end

  def handle_user_message(user, @q_our_soc_networks) do
    Sender.send_text_with_menu(user, :q_our_soc_networks)
    Users.update_state(user, "idle")
  end

  def handle_user_message(user, @q_gift_cert) do
    Sender.send_album(user, :group8)
    Sender.send_text_with_menu(user, :q_gift_cert)
    Users.update_state(user, "idle")
  end

  def handle_user_message(user, text) do
